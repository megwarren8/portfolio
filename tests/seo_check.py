...
# The three book landing pages, added 2026-08-14. These exist because the books
# themselves are 12 to 15 MB and Google will not index them: site: on all three
# subdomains returned zero, probability came back "Crawled, currently not
# indexed", and a Search Console indexing request on the other two failed
# outright while a 10 KB page requested straight afterwards succeeded.
#
# Each of these is a ~14 KB page generated by that book's own
# tools/make_landing.py out of book.src.html. They are checked here rather than
# in the book repos because this is the gate that runs against the live estate,
# and because a landing page that quietly grows past a few hundred KB has
...
def check_landing():
    ...
    for url in LANDING:
        ...
        r.note("%.1f KB" % kb)
        if kb > LANDING_MAX_KB:
            r.fail("landing-size",
                   "%.0f KB, over the %d KB budget; the whole point of this "
                   "page is being small enough to index" % (kb, LANDING_MAX_KB))
        # The failure this is really guarding: a Pages 404 falls back to
        # index.html, which IS the multi-megabyte book. That returns 200 and
        # looks fine until you weigh it. Seen live on 2026-08-14 from a stale
        # edge cache on two of the three.
        if kb > 1000:
            r.fail("landing-is-the-book",
                   "this is serving the book, not the landing page")

        title = first(r"<title>([^<]*)</title>", doc)
        if not title:
            r.fail("title", "missing entirely")
        elif len(title) > TITLE_MAX:
            r.fail("title", "%d chars: %r" % (len(title), title))
        elif "textbook" not in title.lower():
            r.fail("title-keywords", "no 'textbook' in %r" % title)
        ...
        for b in re.findall(
            ...
            try:
                ...
                r.fail("json-ld", "does not parse: %s" % e)
                continue
            for node in (data.get("@graph") or [data]):
                t = node.get("@type")
                types.add(t) if isinstance(t, str) else types.update(t or [])
        if "Book" not in types:
            r.fail("schema-org", "no Book node; found %s"
                   % (", ".join(sorted(types)) or "nothing"))
        else:
            r.note("schema: " + ", ".join(sorted(types)))

        anchors = re.findall(
            r'<a[^>]+href="[^"]*megan-warren\.com/(books|software)/[^"]*"', doc)
        if not anchors:
            r.fail("backlink", "no link to a megan-warren.com money page")
        else:
            r.note("%d links home" % len(anchors))

        text = html.unescape(re.sub(r"<[^>]+>", " ", re.sub(
            r"<(script|style)[\s\S]*?</\1>", " ", doc)))
        if any(d in text for d in DASHES):
            r.fail("body-dash", "contains an em or en dash")
        out.append(r)
    return out
...
def main():
    ...
    if args.only:
        ...
        if args.no_fleet:
            ...
            print("          plus %d fleet subdomains and %d book landing pages"
                  % (len(FLEET), len(LANDING)))
    print()
